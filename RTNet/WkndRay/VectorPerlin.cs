...
namespace WkndRay
{
  public class VectorPerlin
  {
    ...
    private static Vector3[] PerlinGenerate()
    {
      ...
      {
        p[i] = new Vector3(
          -1.0f + (2.0f * RandomService.NextSingle()),
          -1.0f + (2.0f * RandomService.NextSingle()),
          -1.0f + (2.0f * RandomService.NextSingle())).ToUnitVector();
      }
    ...
    private static void Permute(int[] p)
    {
      ...
      {
        int target = Convert.ToInt32(RandomService.NextSingle() * Convert.ToSingle(i + 1));
        int tmp = p[i];
        p[i] = p[target];
        p[target] = tmp;
      }
